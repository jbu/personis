...
def chkspec(t, spec, orig):
    full = dict(
        minset=set(
            range(60)), hourset=set(
            range(24)), domset=set(
                range(31)), monthset=set(
                    range(12)), dowset=set(
                        range(7)))
    now = dict(minset=set([t.tm_min]),
               hourset=set([t.tm_hour]),
               domset=set([t.tm_mday]),
               monthset=set([t.tm_mon]),
               dowset=set([t.tm_wday]))
    print "full = ", full
    print "spec = ", spec
...
'''
        ...
        if m["op"] == "quit":
            ...
            # print "cspec = <%s>" % (cspec)
            # print "min = %s, hour = %s, dom = %s, month = %s, dow = %s" %
            ...
            # print "minset = %s, hourset = %s, domset = %s, monthset = %s,
            # dowset = %s" % (minset,hourset,domset,monthset,dowset)
            m['cronspec'] = dict(
                minset=minset,
                hourset=hourset,
                domset=domset,
