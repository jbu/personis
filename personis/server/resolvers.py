...
class Access:
    ...
    def __init__(self):
        self.resolverlist = {
            "default": self._DefaultResolver,
            "goal": self.GoalResolver,
            "recent": self.RecentResolver}

    def _DefaultResolver(
            self, model=None, component=None, context=None, resolver_args=None):
        ...
        while evcount > 0:
            if ev is not None:
                if not ("goal" in ev.flags):
                    ...
                    ev, evcount = component.getevidence(
                        model=model, context=context, count=evcount - 1)
                    continue
            else:
