...
class Access(base.Access):
    ...
    def subscribe(self,
        ...
        for elt in ("user", "password", "statement"):
            if elt not in subscription:
                raise ValueError(
                    '"%s" key missing from subscription dict' %
                    (elt))
        if len(subscription.items()) != 4:
            raise ValueError(
                'unknown attribute in subscription %s' %
                (subscription))
        # print "token:", token
        self.curcontext = self._getcontextdir(context)
        contextinfo = self.getcontext(context)
        try:
            comps, comps_shelf_fd = base.shelf_open(
                self.curcontext + "/.components", "r")
        except:
            print "can't open ", self.curcontext + "/.components"
            comps = None
        try:
            views, views_shelf_fd = base.shelf_open(
                self.curcontext + "/.views", "r")
        except:
            views = None
        try:
            subs, subs_shelf_fd = base.shelf_open(
                self.curcontext + "/.subscriptions", "w")
        except:
            subs = None
        cidlist = []
        cobjlist = []
        if isinstance(view, StringType):
        ...
        for cid in cidlist:
            if isinstance(cid, type(u"")):
                cid = str(cid)
            if isinstance(cid, StringType):
                if comps is not None:
                    if cid in comps:
                ...
                try:
                    vcomps, vcomps_shelf_fd = base.shelf_open(
                        vcontext + "/.components", "r")
                except:
                    raise ValueError(
                        'context "%s" not in view "%s"' % (cid[-1], repr(view)))
                if cid[-1] in vcomps:
                    ...
                    raise ValueError(
                        'component "%s" not in view "%s"' % (cid[-1], repr(view)))
    ...
    def delete_sub(self, context=None, componentid=None, subname=None):
        self.curcontext = self._getcontextdir(context)
        if isinstance(componentid, type(u'')):
        ...
        try:
            ...
            raise ValueError(
                'no subs db when deleting sub %s for component "%s" in context "%s" not found' %
                (subname, componentid, self.curcontext))
        if componentid not in subs:
            raise ValueError(
                'sub %s for component "%s" in context "%s" not found' %
                (subname, componentid, self.curcontext))
        ...
        try:
            ...
            raise ValueError(
                'cannot delete subname "%s" for component "%s" in context "%s" ' %
                (subname, componentid, self.curcontext))
    ...
    def list_subs(self, context=None, componentid=None):
        ...
        try:
            ...
            raise ValueError(
                'no subs db when listing subs for component "%s" in context "%s" ' %
                (componentid, self.curcontext))
        if componentid not in subs:
            raise ValueError(
                'no subs for component "%s" in context "%s" not found' %
                (componentid, self.curcontext))
        subdict = subs[componentid]
        if subs is not None:
            base.shelf_close(subs, subs_shelf_fd)
