...
# dictionary to hold list of key/value pairs for an element of the  modeldef
keyvals = {}
attrs = []
paths = []  # list of paths
flags = None
...
#	print "dokeyval::", toks
    ...
    if toks[0] in keyvals:
        if isinstance(keyvals[toks[0]], type([])):
...
def dotells(ev, compid):
    ...
    if not all([a in evattrs for a in ev]):
        logging.debug(
            "**** evidence attributes %s must be one of %s" %
            (ev.keys(), repr(evattrs)))
        return
    if "flags" in ev:
        if not isinstance(ev['flags'], type([])):
            logging.debug(
                "**** evidence flags %s must be a list" %
                (ev['flags']))
            return
...
def docomponent(str, loc, toks):
    ...
    for x in required:
        if x not in attrs[0]:
            logging.info(
                "one or more of the required keyvals",
                required,
                "not found for %s",
                toks[1])
    ...
    if 'rule' in attrs[0]:
        if not isinstance(attrs[0]['rule'], type([])):
            rules = [attrs[0]['rule']]
        for rule in rules:
            if not Debug:
                themodel.subscribe(
                    ...
                    subscription=dict(
                        ...
                        statement=rule))
            logging.debug(
                "\tsub:: %s, %s, %s", curcontext, [
                    toks[1]], dict(
                    user="bob", password="qwert", statement=rule))
...
def docontext(str, loc, toks):
    ...
    if not Debug:
        cobj = base.Context(
            Identifier=curcontext.split('/')[-1], Description=attrs[0]['description'])
    logging.debug("\tbase.Context(Identifier='%s', Description='%s')",
                  curcontext.split('/')[-1],
                  attrs[0]['description'])
...
flaglist = Literal('[') + quotedString.setParseAction(removeQuotes) + ZeroOrMore(
    Literal(',') + quotedString.setParseAction(removeQuotes)) + Literal(']')
...
component = Literal('--') + name + Literal(":") + \
    keyval_list + Optional(Literal(',') + evidencelist)
view = Literal('==') + name + Literal(":") + \
    path + ZeroOrMore(Literal(",") + path)
mdef = OneOrMore(
    context + ZeroOrMore(component) + ZeroOrMore(view)) + Literal('$$')
