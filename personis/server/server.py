...
class Server:
    ...
    def __init__(self, modeldir='models', adminsfile='admins.yaml', clients=None,
                 access_tokens='./access_tokens.dat', client_secrets='client_secrets_google.json'):
    ...
    def list_clients(self):
        ...
        base_path = os.path.dirname(os.path.abspath(__file__))
        return open(
            os.path.join(base_path, 'html', 'list_clients.html')).read()
    ...
    @cherrypy.expose
    def authorize(self, client_id, redirect_uri, scope, access_type,
                  response_type='code', approval_prompt='auto', state=None):
        """
        ...
        if cli['redirect_uri'] != redirect_uri:
            logging.debug(
                "redirect uris don't match expected %s got %s",
                cli['redirect_uri'],
                redirect_uri)
    ...
    def allow(self):
        ...
        access_tokens[val_key] = {
            'timestamp': time.time(),
            'userid': usrid,
            'client_id': cherrypy.session['client_id'],
            'type': 'authorization_code',
            'expires': time.time() + 600}
        # self.access_tokens.sync()
        logging.info('access tokens: %s',
                     repr([i for i in access_tokens.keys()]))

        redr = cli['redirect_uri']
        um = active.Access(model=usrid, modeldir=self.modeldir, user=usrid)
        cherrypy.session['um'] = um

        result = um.registerapp(
            app=cherrypy.session['client_id'],
            desc=cli['friendly_name'],
            realm='oauth')
    ...
    @cherrypy.expose
    def request_token(self, client_id, client_secret, grant_type,
                      code=None, redirect_uri=None, scope=None, refresh_token=None):
        """
        ...
        """
        logging.debug(
            'code %s, redirui %s, clientid %s, clientsec %s, scope %s, granttype %s, refreshtoken %s' %
            (code,
             redirect_uri,
             client_id,
             client_secret,
    ...
    def default(self, *args, **kargs):
        ...
        # 3) Web clients that might be using this to add or delete apps for a
        # user. These can have a local session.
        ...
        logging.debug(
            ...
            'Authorization' in cherrypy.request.headers)
        ...
        # we're from a web client using oauth.
        if 'Authorization' in cherrypy.request.headers:
            ...
            if access_tokens[access_token]['client_id'] not in apps:
                logging.debug(
                    'client for access token not in model %s, %s',
                    access_token,
                    access_tokens[access_token]['client_id'])
                raise cherrypy.HTTPError(
                    401,
                    'client for access token not in model')
        ...
        try:
            ...
            if args[0] == 'access':
                ...
                result = um.getpermission(
                    context=pargs['context'],
                    componentid=pargs['componentid'],
                    app=pargs['app'])
            elif args[0] == 'setpermission':
                result = um.setpermission(
                    context=pargs['context'],
                    componentid=pargs['componentid'],
                    app=pargs['app'],
            ...
            # Repackage result code with error values IF there is a version
            # string.
            if 'version' in pargs:
                new_result = {}
...
def runServer(modeldir, config, admins, clients, tokens, loglevel=logging.INFO,
              exit_queue=None, client_secrets='client_secrets_google.json'):
