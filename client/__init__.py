...
def _MkModel( model=None, modelserver=None, user=None, password=None, description=None, debug=0):
    if modelserver == None:
...
component_types = ["attribute", "activity", "knowledge", "belief", "preference", "goal"]
value_types = ["string", "number", "boolean", "enum", "JSON"]
evidence_types = ["explicit", # given by the user  (given)
...
class component:
    ...
    def __init__(self, **kargs):
        ...
        if self.Identifier == None:
            return None
        if not self.component_type in component_types:
            raise TypeError, "bad component type %s"%(self.component_type)
        if not self.value_type in value_types:
...
class evidence:
    ...
    def __init__(self, **kargs):
        ...
        for k,v in kargs.items():
            self.__dict__[k] = v
        if not self.evidence_type in evidence_types:
...
class access(object):
    ...
    """
    def __init__(self, uri = None, credentials = None, http = None, debug=0, test=True):
        c = connection(uri, credentials, http)
        self.__init__(c, debug, test)
